...
A target may be one of three variants: "ignored", if not relevant as described before, "connected", if the device's relay port is used for it, or "Modbus" for a remote "Smartdose" socket addressed via Modbus.
